...
wtree2_txn_t* _mongolite_get_write_txn(mongolite_db_t *db, gerror_t *error) {
    ...
    return wtree2_txn_begin(db->wdb, true, error);
}
 ...
 * we reuse a cached transaction via wtree2_txn_renew() which only
...
wtree2_txn_t* _mongolite_get_read_txn(mongolite_db_t *db, gerror_t *error) {
    ...
    if (MONGOLITE_LIKELY(db->read_txn_pool != NULL)) {
        /* Get underlying wtree txn for renew */
        wtree_txn_t *wtxn = wtree2_txn_get_wtree(db->read_txn_pool);
        int rc = wtree_txn_renew(wtxn, error);
    ...
    wtree2_txn_t *txn = wtree2_txn_begin(db->wdb, false, error);
...
void _mongolite_release_read_txn(mongolite_db_t *db, wtree2_txn_t *txn) {
    ...
    if (MONGOLITE_LIKELY(txn == db->read_txn_pool)) {
        wtree_txn_t *wtxn = wtree2_txn_get_wtree(txn);
        wtree_txn_reset(wtxn);
    } else {
...
int _mongolite_commit_if_auto(mongolite_db_t *db, wtree2_txn_t *txn, gerror_t *error) {
    ...
    if (MONGOLITE_LIKELY(!db->in_transaction)) {
        return wtree2_txn_commit(txn, error);
    }
    return MONGOLITE_OK;
}
 ...
 *
 * NOTE: With wtree2, doc_count is maintained internally by the tree.
 * This function now just updates the schema for persistence.
 * The cached count comes from wtree2_tree_count().
 * ============================================================ */
...
int _mongolite_update_doc_count_txn(mongolite_db_t *db, wtree2_txn_t *txn,
    ...
    /* Get underlying wtree txn for schema operations */
    wtree_txn_t *wtxn = wtree2_txn_get_wtree(txn);

    /* Read schema entry using provided transaction */
    const void *value;
    size_t value_size;
    int rc = wtree_get_txn(wtxn, db->schema_tree, collection, strlen(collection),
    ...
    /* Get the current count from wtree2 (source of truth) */
    wtree2_tree_t *tree = _mongolite_tree_cache_get(db, collection);
    if (tree) {
        entry.doc_count = wtree2_tree_count(tree);
    } else {
    ...
    rc = wtree_update_txn(wtxn, db->schema_tree,
...
int mongolite_begin_transaction(mongolite_db_t *db) {
    ...
    db->current_txn = wtree2_txn_begin(db->wdb, true, &local_error);
    if (!db->current_txn) {
        _mongolite_unlock(db);
...
int mongolite_commit(mongolite_db_t *db) {
    ...
    gerror_t local_error = {0};
    int rc = wtree2_txn_commit(db->current_txn, &local_error);
...
int mongolite_sync(mongolite_db_t *db, bool force, gerror_t *error) {
    ...
    return wtree2_db_sync(db->wdb, force, error);
}
