...
bool mongolite_cursor_next(mongolite_cursor_t *cursor, const bson_t **doc) {
    ...
    if (cursor->position == 0) {
        has_entry = wtree3_iterator_first(cursor->iter);
    } else {
        has_entry = wtree3_iterator_next(cursor->iter);
    }
    ...
    while (has_entry) {
        ...
        if (!wtree3_iterator_value(cursor->iter, &value, &value_size)) {
            has_entry = wtree3_iterator_next(cursor->iter);
            continue;
        }
...
mongolite_cursor_t* _mongolite_cursor_create_with_txn(mongolite_db_t *db,
    ...
    cursor->iter = wtree3_iterator_create_with_txn(tree, txn, error);
    if (!cursor->iter) {
        free(cursor->collection_name);
        free(cursor);
        return NULL;
