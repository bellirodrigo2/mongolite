...
int wtree3_index_load_metadata(wtree3_tree_t *tree,
    ...
    /* Now reconstruct the index by adding it to the tree */
    wtree3_index_config_t config = {
        .name = index_name,
        .key_fn = key_fn,
        .user_data = user_data,
        .user_data_cleanup = persistence->serialize ? NULL : NULL,  /* Will be set below */
        .unique = unique,
        .sparse = sparse,
        .compare = NULL,  /* TODO: Should we persist compare function pointer? */
        .persistence = persistence
    };

    /* Note: We need to open the existing index DBI, not create a new one */
    char *idx_tree_name = build_index_tree_name(tree->name, index_name);
    if (!idx_tree_name) {
        if (user_data && persistence->deserialize) {
            /* We don't have cleanup function, so this might leak */
            /* TODO: Should persistence include cleanup? */
        }
        set_error(error, WTREE3_LIB, WTREE3_ENOMEM, "Failed to allocate tree name");
        return WTREE3_ENOMEM;
    }
    ...
    idx->user_data_cleanup = NULL;  /* Set by caller if needed */
    idx->unique = unique;
    idx->sparse = sparse;
    idx->compare = NULL;
